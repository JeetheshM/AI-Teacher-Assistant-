...
class ApiClient {
  ...
  private normalizeLesson(data: any, req: LessonGenerationRequest): LessonPlan {
    return {
      id: data.id || data.lesson_id || Date.now(),
      title: data.title || `${req.topic} Lesson Plan`,
      subject: req.subject,
      grade: req.grade,
      duration_minutes: req.duration_minutes,
      difficulty: req.difficulty,
