...
const AppInner: React.FC = () => {
  ...
  const handleLoadSavedLesson = async (lessonId: string) => {
    setIsGenerating(true);
    setGenerationError(null);
    try {
      const lessonResult = await apiClient.getLesson(lessonId);
      setTeachingPackage({
        lesson: lessonResult,
        activity: MOCK_ACTIVITY,
        quiz: MOCK_QUIZ,
        document: documentState || undefined,
        sources: lessonResult.sources_used || MOCK_SOURCES,
        transformations: {
          simplify: MOCK_TRANSFORMATIONS.simplify,
          younger_level: MOCK_TRANSFORMATIONS.younger_level,
          analogy: MOCK_TRANSFORMATIONS.analogy,
      ...
      addToast('Lesson loaded from library!', 'success');
    } catch (err: any) {
      addToast(`Could not load lesson: ${err.message}`, 'error');
    } finally {
      setIsGenerating(false);
    }
  };
  ...
  return (
    <div className="app-shell">
      <Sidebar
        ...
        onLoadLesson={handleLoadSavedLesson}
      />
